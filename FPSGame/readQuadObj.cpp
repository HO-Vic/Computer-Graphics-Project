...
void readQuadObj(const std::string objfilename, std::vector<glm::vec3>& vertex, std::vector<glm::vec3>& vertexNomal)
{
	...
	while (inFile >> std::noskipws >> head[0]) {
		...
		if (head[0] == 'f') {
			...
			if (head[1] == ' ') {
				...
				inFile >> std::skipws >> vertexFaceNum[3];
				inFile >> std::skipws >> tempC;
				inFile >> std::skipws >> tempi;
				inFile >> std::skipws >> tempC;
				inFile >> std::skipws >> vertexNomalFaceNum[3];
				glm::vec4 vertexFaceTemp = { vertexFaceNum[0], vertexFaceNum[1], vertexFaceNum[2], vertexFaceNum[3] };
				glm::vec4 vertexNomalFaceTemp = { vertexNomalFaceNum[0], vertexNomalFaceNum[1], vertexNomalFaceNum[2],vertexNomalFaceNum[3] };
	...
	for (auto iter = vertexNomalFace.begin(); iter < vertexNomalFace.end(); iter++) {
		...
		vertexNomal.push_back(vertexNomalData[(int)(iter->w) - 1]);
	}
	delete[] vertexData;
